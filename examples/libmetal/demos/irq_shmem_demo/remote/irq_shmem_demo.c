 ...
 *  4. Wait for remote IRQ notification to receive message.
 ...
 *     shared buffer.
 *  7. Kick IRQ to notify there is a message written to the shared memory
 *     if it echos back the message.
 *  8. Repeat 4.
 *  9. Clean up: disable IRQ interrupt, deregister the IRQ interrupt handler.
 *
 ...
 *	  This task will:
 *	  * Wait for IRQ interrupt from the remote
...
int demo(void *arg)
{
	...
	if (ret) {
		LPERROR("Failed to set task to system.\n");
		goto out;
	}
...
out:
	LPRINTF("IRQ with shared memory demo finished with exit code: %i.\n",
		ret);
